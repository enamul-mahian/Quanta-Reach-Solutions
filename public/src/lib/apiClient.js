...
const hydrateTimestamp = (value) => {
    ...
    return { seconds: Math.floor(ms / 1000), nanoseconds: 0, toDate: () => new Date(date), toMillis: () => ms };
};
const hydrate = (value, key = '') => {
    ...
    if (typeof value === 'string' && /At$/.test(key)) return hydrateTimestamp(value);
    return value;
};
const request = async (path, options = {}) => {
    ...
    return hydrate(payload.data ?? payload);
};
export const api = {
...
export const contentApi = {
    list: (type, includeAll = false) => api.get(`/content/${type}${includeAll ? '?include_all=1' : ''}`),
    show: (type, idOrSlug) => api.get(`/content/${type}/${encodeURIComponent(idOrSlug)}`),
    create: (type, data) => api.post(`/content/${type}`, { data }),
    publicCreate: (type, data) => api.post(`/public/${type}`, { data }),
    update: (type, id, data) => api.patch(`/content/${type}/${encodeURIComponent(id)}`, { data }),
    remove: (type, id) => api.delete(`/content/${type}/${encodeURIComponent(id)}`),
};
