  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 relative z-10">
        <SectionHeading 
          badge={language === 'en' ? 'OUR SYSTEMATIC PROCESS' : 'আমাদের কাজের পদ্ধতি'}
          title={language === 'en' ? 'Ideas Into Impact' : 'ধারণা থেকে বাস্তব প্রবৃদ্ধি'}
          subtitle={
            language === 'en' 
              ? 'A meticulous, enterprise-grade 10-step digital engineering pipeline tailored to craft robust digital innovations.' 
              : 'একটি সূক্ষ্ম, এন্টারপ্রাইজ-গ্রেডের ১০টি ধাপের ডিজিটাল ইঞ্জিনিয়ারিং পাইপলাইন যা শক্তিশালী ডিজিটাল উদ্ভাবন নিশ্চিত করে।'
          }
          align="left"
        />

        <div className="relative mt-20 md:mt-32" ref={stepsRef}>
          {/* গ্লোবাল ভার্টিকাল টাইমলাইন লাইন */}
          <div className="absolute left-6 md:left-12 top-0 w-[2px] h-full bg-white/5 overflow-hidden">
            <div 
              ref={progressLineRef}
              className="absolute top-0 left-0 w-full h-full bg-gradient-to-b from-[#168BFF] via-[#7457FF] to-[#168BFF] origin-top shadow-[0_0_15px_#168BFF]"
              style={{ willChange: 'transform' }}
            />
          </div>

          {/* স্টেপগুলোর গ্রিড/লিস্ট কন্টেইনার */}
          <div className="flex flex-col gap-10 md:gap-14 w-full">
            {processSteps.map((step) => (
              <div 
                key={step.id}
                className="process-step group relative pl-16 md:pl-28 pr-6 py-8 md:py-10 rounded-2xl border border-white/[0.01] bg-white/[0.01] hover:bg-white/[0.025] hover:border-white/[0.06] transition-all duration-700 shadow-2xl overflow-hidden backdrop-blur-sm"
              >
                {/* টাইমলাইন ডট ও অ্যাক্টিভ রিপল ইফেক্ট */}
                <div className="absolute left-6 md:left-12 -translate-x-1/2 top-11 md:top-14 w-4.5 h-4.5 rounded-full bg-[#071426] border-2 border-white/20 group-hover:border-[#168BFF] group-[.is-active]:border-[#168BFF] flex items-center justify-center transition-all duration-500 z-20">
                  {/* সেন্টার ডট */}
                  <div className="w-1.5 h-1.5 rounded-full bg-white/30 group-hover:bg-[#168BFF] group-[.is-active]:bg-[#168BFF] transition-all duration-500" />
                  
                  {/* ব্যাকগ্রাউন্ড ডাইনামিক রিপল অ্যানিমেশন (শুধুমাত্র একটিভ বা হভার স্টেটে পালস করবে) */}
                  <div className="absolute inset-[-4px] rounded-full bg-[#168BFF]/20 animate-ping opacity-0 group-hover:opacity-100 group-[.is-active]:opacity-100 duration-1000 pointer-events-none" />
                </div>

                {/* ব্যাকগ্রাউন্ড লার্জ মেটাল আইডি টেক্সট (UI/UX Depth) */}
                <div className="step-id select-none text-7xl md:text-9xl font-black font-mono leading-none tracking-tighter opacity-[0.02] group-hover:opacity-[0.08] group-[.is-active]:opacity-[0.12] text-[#168BFF] absolute right-4 top-4 md:right-8 md:top-8 transition-all duration-700 pointer-events-none" style={{ willChange: 'opacity' }}>
                  {step.id}
                </div>

                {/* স্টেপ কন্টেন্ট */}
                <div className="step-content-reveal max-w-3xl">
                  {/* ক্যাটাগরি লেবেল / আইডি */}
                  <span className="inline-block text-xs font-mono font-bold tracking-widest text-[#168BFF] uppercase mb-2 opacity-60 group-hover:opacity-100 group-[.is-active]:opacity-100 transition-opacity duration-500">
                    {language === 'en' ? `Phase ${step.id}` : `ধাপ ${step.id}`}
                  </span>

                  {/* হেডিং */}
                  <h3 className="text-white/60 group-hover:text-white group-[.is-active]:text-white text-2xl md:text-4xl font-bold tracking-tight mb-3 transition-colors duration-500">
                    {language === 'en' ? step.title.en : step.title.bn}
                  </h3>

                  {/* ডেসক্রিপশন */}
                  <p className="text-white/40 group-hover:text-white/70 group-[.is-active]:text-white/80 text-sm md:text-lg leading-relaxed font-light transition-colors duration-500">
                    {language === 'en' ? step.desc.en : step.desc.bn}
                  </p>

                  {/* ডেলিভারেবলস সেকশন (Sub-points) */}
                  <div className="mt-6">
                    <p className="text-xs font-mono text-white/20 uppercase tracking-widest mb-3">
                      {language === 'en' ? 'Core Deliverables' : 'প্রধান ডেলিভারেবলস'}
                    </p>
                    <div className="flex flex-wrap gap-2.5">
                      {(language === 'en' ? step.deliverables.en : step.deliverables.bn).map((del, index) => (
                        <span 
                          key={index}
                          className="step-deliverable flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] md:text-xs font-mono font-medium tracking-wide bg-white/[0.02] border border-white/[0.04] text-white/40 group-hover:text-white/80 group-hover:border-white/10 group-[.is-active]:text-white group-[.is-active]:border-[#168BFF]/30 group-[.is-active]:bg-[#168BFF]/5 transition-all duration-500"
                        >
                          <span className="w-1 h-1 rounded-full bg-[#168BFF] group-hover:bg-cyan-400 group-[.is-active]:bg-cyan-400 transition-all duration-500" />
                          {del}
                        </span>
                      ))}
                    </div>
      ...
      {/* ব্যাকগ্রাউন্ড ডেকোরেটিভ গ্লো (Subtle Ambient Glow) */}
      <div className="absolute top-1/2 right-0 w-[350px] h-[700px] bg-[#168BFF]/3 filter blur-[130px] rounded-l-full pointer-events-none -z-10" />
      <div className="absolute top-1/4 left-0 w-[250px] h-[500px] bg-[#7457FF]/2 filter blur-[100px] rounded-r-full pointer-events-none -z-10" />
    </section>
  );
};