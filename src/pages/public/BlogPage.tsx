// =========================================================================
// MetaFore Technologies - Blog & Insights Page (Premium Animated Edition)
// =========================================================================
...
export const BlogPage: React.FC = () => {
  ...
  return (
    <div ref={pageRef} className="w-full flex flex-col bg-[#071426]">
      <Helmet>
        <title>{t.navigation.blog} | MetaFore Technologies</title>
        <meta name="description" content={language === 'en' ? 'Latest insights, trends, and news on technology and business growth.' : 'টেকনোলজি এবং বিজনেস গ্রোথ সংক্রান্ত সর্বশেষ ইনসাইটস, ট্রেন্ড এবং খবর।'} />
      </Helmet>

      {/* Hero Section */}
      <section className="pt-32 pb-16 lg:pt-40 lg:pb-24 border-b border-white/5 bg-[#0D1D33] relative">
        ...
        <Container className="text-center max-w-3xl">
          <div ref={heroRef}>
            <SectionHeading 
              ...
              title={language === 'en' ? 'Insights & Stories' : 'ইনসাইটস এবং স্টোরিজ'}
              subtitle={language === 'en' 
                ? 'Expert thoughts on technology, innovation, and digital transformation for modern businesses.' 
                : 'আধুনিক ব্যবসার জন্য টেকনোলজি, ইনোভেশন এবং ডিজিটাল রূপান্তর নিয়ে আমাদের এক্সপার্ট মতামত।'}
          ...
          <div ref={searchRef} className="relative max-w-lg mx-auto">
            ...
            <input 
              type="text" 
              placeholder={language === 'en' ? 'Search blogs...' : 'ব্লগ সার্চ করুন...'}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-[#071426] border border-white/5 rounded-full py-4 pl-12 pr-6 text-white focus:outline-none focus:border-[#168BFF] focus:shadow-[0_0_15px_rgba(22,139,255,0.15)] transition-all shadow-2xl"
            />
          </div>
        </Container>
      </section>
      ...
      {/* Blog Grid */}
      <section className="py-20 lg:py-28 bg-[#071426]">
        <Container>
          <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredPosts.map((post) => (
              <div key={post.id} className="blog-card bg-[#0D1D33]/50 backdrop-blur-sm rounded-2xl overflow-hidden group hover:-translate-y-2 transition-all duration-500 flex flex-col h-full border border-white/5 hover:border-[#168BFF]/30 hover:shadow-[0_20px_50px_rgba(22,139,255,0.08)]">
                <div className="relative h-56 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 z-10">
                    <span className="px-3 py-1 text-xs font-semibold rounded-full bg-[#071426]/80 text-[#168BFF] backdrop-blur-md border border-white/10">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <div className="flex items-center gap-4 text-xs font-medium text-white/40 mb-4">
                    <div className="flex items-center gap-1.5">
                      <Calendar className="w-3.5 h-3.5 text-[#9C5CFF]" />
                      <span>{post.date}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-[#9C5CFF]" />
                      <span>{language === 'en' ? post.readTime.en : post.readTime.bn}</span>
                    </div>
                  </div>

                  <h3 className="text-xl font-bold text-white mb-3 group-hover:text-[#168BFF] transition-colors line-clamp-2">
                    <Link to={`/blog/${post.slug}`}>
                      {language === 'en' ? post.title.en : post.title.bn}
                    </Link>
                  </h3>
                  
                  <p className={`text-white/50 text-sm leading-relaxed mb-6 line-clamp-3 flex-grow ${language === 'bn' ? 'leading-loose' : ''}`}>
                  ...
                  <div className="pt-4 border-t border-white/5 mt-auto">
                    <Link 
                      to={`/blog/${post.slug}`}
                      className="inline-flex items-center text-sm font-semibold text-[#168BFF] hover:text-[#36A3FF] transition-colors"
                    >
                      {t.common.readMore}
          ...
          {filteredPosts.length === 0 && (
            <div className="text-center py-20 text-white/30">
              {language === 'en' ? 'No blogs found matching your search.' : 'আপনার সার্চের সাথে মিলছে এমন কোনো ব্লগ পাওয়া যায়নি।'}
            </div>
          )}
        </Container>
      </section>
