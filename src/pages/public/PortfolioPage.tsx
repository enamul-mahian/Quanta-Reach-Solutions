// =========================================================================
// MetaFore Technologies - Portfolio & Case Studies Page (Premium Animated Edition)
// =========================================================================
...
export const PortfolioPage: React.FC = () => {
  ...
  const filteredItems = portfolioItems.filter(item => {
    ...
    if (activeFilter === 'Global' || activeFilter === 'Local') return item.clientType === activeFilter;
  ...
  return (
    <div ref={pageRef} className="w-full flex flex-col bg-[#071426]">
      <Helmet>
        <title>{t.navigation.portfolio} | MetaFore Technologies</title>
        <meta name="description" content={language === 'en' ? 'Explore our digital masterpieces and success stories.' : 'আমাদের ডিজিটাল কাজগুলোর নমুনা এবং সফলতার গল্পগুলো দেখুন।'} />
      </Helmet>

      {/* Hero Section */}
      <section className="relative pt-32 pb-16 lg:pt-40 lg:pb-24 overflow-hidden border-b border-white/5 bg-[#0D1D33]">
        <div className="absolute top-0 right-1/4 w-[500px] h-[500px] bg-purple-accent/10 rounded-full blur-[120px] pointer-events-none"></div>
        <Container className="relative z-10 text-center max-w-4xl">
          <div ref={heroRef}>
            <SectionHeading 
              badge={t.navigation.portfolio}
              title={language === 'en' ? 'Our Digital Masterpieces' : 'আমাদের ডিজিটাল মাস্টারপিস'}
              subtitle={language === 'en' 
                ...
                : 'আমাদের সেরা কাজগুলো দেখুন, যা আমাদের কারিগরি দক্ষতা, ডিজাইন চিন্তাধারা এবং সমস্যা সমাধানের ক্ষমতা প্রমাণ করে।'}
              className="mx-auto"
            />
          </div>
        </Container>
      ...
      <section className="py-20 lg:py-28 relative bg-[#071426]">
        <Container>
          
          {/* Filters */}
          <div className="flex flex-col md:flex-row items-center justify-between mb-12 gap-6">
            <div className="flex items-center gap-2 text-white/50 font-medium">
              <Filter className="w-5 h-5 text-[#168BFF]" />
              <span>{language === 'en' ? 'Filter by:' : 'ফিল্টার করুন:'}</span>
            </div>
            
            <div ref={filtersRef} className="flex flex-wrap justify-center md:justify-end gap-3">
                ...
                <button
                  ...
                  className={`px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 border ${
                    activeFilter === category.id
                      ...
                      : 'bg-[#0D1D33] text-white/50 border-white/5 hover:border-[#168BFF] hover:text-white'
                  }`}
                >
                  {language === 'en' ? category.label.en : category.label.bn}
                </button>
              ))}
            </div>
          </div>
          ...
          <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredItems.map((item) => (
              <div key={item.id} className="portfolio-card group relative rounded-2xl overflow-hidden bg-[#0D1D33]/50 backdrop-blur-sm border border-white/5 hover:border-[#168BFF]/30 transition-all duration-500 flex flex-col h-full hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(22,139,255,0.08)]">
                ...
                <div className="relative h-60 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 z-20 flex gap-2">
                    <span className="px-3 py-1 text-xs font-semibold rounded-full bg-[#071426]/80 text-white backdrop-blur-md border border-white/10">
                      {item.category}
                    </span>
                    <span className={`px-3 py-1 text-xs font-semibold rounded-full backdrop-blur-md border border-white/10 ${
                      item.clientType === 'Global' ? 'bg-[#168BFF]/80 text-white' : 'bg-purple-accent/80 text-white'
                    }`}>
                      {item.clientType}
                    </span>
                  </div>
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <h3 className="text-xl font-bold text-white mb-4 group-hover:text-[#168BFF] transition-colors line-clamp-2">
                    {language === 'en' ? item.title.en : item.title.bn}
                  </h3>
                  
                  {/* Success Metrics */}
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#168BFF]/10 border border-[#168BFF]/20 text-[#168BFF] mb-6 w-fit">
                  ...
                  <div className="pt-6 border-t border-white/5 mt-auto flex justify-between items-center">
                    <Link 
                      ...
                      className="inline-flex items-center text-white/40 hover:text-white font-medium transition-colors text-sm"
                    >
                      {t.common.readMore}
                    ...
                    <Link 
                      to={`/portfolio/${item.slug}`}
                      className="w-10 h-10 rounded-full bg-[#0D1D33] border border-white/5 flex items-center justify-center text-white/50 hover:text-white hover:bg-[#168BFF] hover:border-[#168BFF] transition-all shrink-0"
          ...
          {filteredItems.length === 0 && (
            <div className="text-center py-20">
              <h3 className="text-xl text-white/40">
                {language === 'en' ? 'No projects found for this category.' : 'এই ক্যাটাগরিতে কোনো প্রজেক্ট পাওয়া যায়নি।'}
              </h3>
            </div>
          )}
      ...
      {/* CTA Section - Placed with Fixed Text Color Button */}
      <section className="py-24 relative overflow-hidden bg-[#168BFF]">
        ...
        <Container className="relative z-10 text-center">
          <h2 className="text-4xl md:text-6xl font-black text-white mb-8 tracking-tighter uppercase">
            {language === 'en' ? 'Let’s Build Your Success Story' : 'চলুন আপনার সফলতার গল্প তৈরি করি'}
          </h2>
