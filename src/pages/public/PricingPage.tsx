// =========================================================================
// MetaFore Technologies - Detailed Pricing Page (Premium Animated Edition)
// =========================================================================
...
export const PricingPage: React.FC = () => {
  ...
  return (
    <div ref={pageRef} className="w-full flex flex-col bg-[#071426]">
      <Helmet>
        <title>{t.navigation.pricing} | MetaFore Technologies</title>
        <meta name="description" content={language === 'en' ? 'Transparent pricing plans for web and app development.' : 'ওয়েব এবং অ্যাপ ডেভেলপমেন্টের জন্য স্বচ্ছ প্রাইসিং প্যাকেজ।'} />
      </Helmet>

      {/* Hero Section */}
      <section className="relative pt-32 pb-16 lg:pt-40 lg:pb-24 overflow-hidden border-b border-white/5 bg-[#0D1D33]">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-[#168BFF]/10 rounded-full blur-[120px] pointer-events-none"></div>
        <Container className="relative z-10 text-center max-w-4xl">
          <div ref={heroRef}>
            <SectionHeading 
              badge={t.navigation.pricing}
              title={language === 'en' ? 'Clear & Transparent Pricing' : 'স্বচ্ছ এবং সাশ্রয়ী প্যাকেজসমূহ'}
              subtitle={language === 'en' 
                ? 'No hidden fees. We provide value-driven solutions tailored for both growing local businesses and global enterprises.' 
                : 'কোনো লুকানো চার্জ নেই। আমরা দেশীয় ব্যবসা এবং গ্লোবাল এন্টারপ্রাইজ উভয়ের জন্যই মানসম্মত সলিউশন প্রদান করি।'}
          ...
          <div className="flex justify-center">
            <div className="bg-[#071426] p-1.5 rounded-full border border-white/10 inline-flex relative shadow-2xl">
              <button
                onClick={() => setClientType('local')}
                className={`relative z-10 px-8 py-3 rounded-full font-bold uppercase tracking-wider text-xs transition-all duration-300 ${
                  clientType === 'local' ? 'text-white' : 'text-white/40 hover:text-white'
                }`}
              >
                {language === 'en' ? 'Local (BDT)' : 'লোকাল (BDT)'}
              </button>
              <button
                onClick={() => setClientType('global')}
                className={`relative z-10 px-8 py-3 rounded-full font-bold uppercase tracking-wider text-xs transition-all duration-300 ${
                  clientType === 'global' ? 'text-white' : 'text-white/40 hover:text-white'
                }`}
              >
                {language === 'en' ? 'Global (USD)' : 'গ্লোবাল (USD)'}
              </button>
              
              <div 
                className="absolute top-1.5 bottom-1.5 rounded-full bg-[#168BFF] transition-all duration-300 ease-in-out shadow-lg shadow-[#168BFF]/20"
                style={{
      ...
      <section className="py-20 lg:py-32 relative bg-[#071426]">
        <Container>
          <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
            {currentPlans.map((plan) => (
              <div 
                key={plan.id}
                className={`pricing-card relative bg-[#0D1D33]/50 backdrop-blur-sm p-8 md:p-10 rounded-3xl flex flex-col border transition-all duration-500 hover:-translate-y-2 group ${
                  plan.isPopular ? 'border-[#168BFF] shadow-[0_20px_50px_rgba(22,139,255,0.1)] md:-mt-6 md:mb-6 z-10' : 'border-white/5 hover:border-white/20'
                }`}
              >
                {plan.isPopular && (
                  <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#168BFF] text-[#071426] px-5 py-1.5 rounded-full text-[10px] font-black tracking-[0.2em] uppercase shadow-xl">
                    {language === 'en' ? 'Most Popular' : 'সেরা পছন্দ'}
                  </div>
                )}

                <div className="mb-8">
                  <h3 className="text-2xl md:text-3xl font-black text-white mb-3 tracking-tight uppercase">
                    {language === 'en' ? plan.name.en : plan.name.bn}
                  </h3>
                  <p className={`text-white/40 text-sm leading-relaxed ${language === 'bn' ? 'leading-normal' : ''}`}>
                    {language === 'en' ? plan.description.en : plan.description.bn}
                  </p>
                </div>

                <div className="mb-8 pb-8 border-b border-white/5">
                  <span className="text-[#168BFF] text-[10px] font-black uppercase tracking-[0.2em] mb-2 block">
                    {plan.price === 'Custom' ? '' : t.pricing.startingAt}
                  </span>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8 bg-[#071426]/50 p-5 rounded-2xl border border-white/5">
                  <div>
                    <span className="block text-[10px] font-bold text-white/30 uppercase tracking-widest mb-1">{language === 'en' ? 'Delivery' : 'ডেলিভারি'}</span>
                    <span className="block text-sm text-white font-bold">{language === 'en' ? plan.delivery.en : plan.delivery.bn}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-white/30 uppercase tracking-widest mb-1">{language === 'en' ? 'Revisions' : 'রিভিশন'}</span>
                    <span className="block text-sm text-white font-bold">{language === 'en' ? plan.revisions.en : plan.revisions.bn}</span>
                  </div>
                </div>

                <div className="flex-grow">
                  <span className="block text-xs font-black text-white uppercase tracking-widest mb-6 opacity-60">
                    {language === 'en' ? 'Included Features' : 'প্যাকেজের অন্তর্ভুক্ত:'}
                  </span>
                  <ul className="space-y-4 mb-10">
                    {(language === 'en' ? plan.features.en : plan.features.bn).map((feature, idx) => (
                      <li key={idx} className="flex items-start gap-3 group/item">
                        <CheckCircle2 className="w-5 h-5 text-[#168BFF] shrink-0 mt-0.5 group-hover/item:scale-110 transition-transform" />
                ...
                <Link to="/request-quote" className="mt-auto">
                  <Button 
                    ...
                    fullWidth
                    className="rounded-full py-4 font-black uppercase tracking-widest text-xs"
                  >
      ...
      {/* FAQs Section */}
      <section className="py-24 bg-[#0D1D33] border-y border-white/5 relative overflow-hidden">
        <Container className="relative z-10 max-w-5xl">
          <div ref={faqRef}>
            <SectionHeading 
              title={language === 'en' ? 'Pricing FAQ' : 'প্রাইসিং নিয়ে জিজ্ঞাসা'}
              subtitle={language === 'en' 
                ? 'Everything you need to know about our project ownership and payment cycles.' 
                : 'প্রজেক্টের মালিকানা এবং পেমেন্ট সিস্টেম সম্পর্কে বিস্তারিত তথ্য।'}
            />

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-16">
              {faqs.map((faq, index) => (
                <div key={index} className="faq-item bg-[#071426]/50 p-8 rounded-3xl border border-white/5 hover:border-[#168BFF]/20 transition-all duration-300">
        ...
        <Container className="relative z-10 text-center">
          ...
          <p className="text-white/90 text-xl mb-12 max-w-2xl mx-auto font-medium">
            {language === 'en' 
              ...
              : 'আপনার নির্দিষ্ট রিকোয়ারমেন্টস নিয়ে আলোচনা করতে এবং একটি কাস্টম প্রপোজাল পেতে আমাদের সাথে যোগাযোগ করুন।'}
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-6">
            ...
            <a href="https://wa.me/8801983398333" target="_blank" rel="noopener noreferrer">
              <Button variant="secondary" size="lg" className="border-white/30 bg-[#071426] text-white hover:bg-black font-black uppercase tracking-widest px-12 py-6 rounded-full transition-all hover:scale-105 active:scale-95 shadow-2xl">
                {t.common.whatsappUs}
              </Button>
            </a>
          </div>
