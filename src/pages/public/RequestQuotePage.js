...
import { isFirebaseReady } from '/src/lib/firebase.js';
import { isCloudinaryConfigured } from '/src/lib/env.js';
...
export const RequestQuotePage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        let fileUrl = '';
        try {
            // A static deployment remains usable before the optional Firebase backend is connected.
            if (!isFirebaseReady()) {
            ...
            // Upload an attachment only when Cloudinary has been configured.
            ...
            const payload = {
                ...
                projectDescription: selectedFile && !isCloudinaryConfigured()
                    ? `${formData.description}\n\nAttachment selected but not uploaded: ${selectedFile.name}. Please request it from the client.`
                    : formData.description,
            ...
            if (selectedFile && !isCloudinaryConfigured()) {
                toast(language === 'en'
                    ? 'Attachment upload is not configured yet. The request will be saved with the filename so the team can ask for it.'
                    : 'Attachment upload এখনো configured নয়। ফাইলের নামসহ অনুরোধটি সেভ হবে, পরে টিম ফাইলটি চাইতে পারবে।', { id: 'processToast', icon: 'ℹ️' });
            }
