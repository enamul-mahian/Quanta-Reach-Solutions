...
/**
 * QUANTA REACH SOLUTIONS - MOTION DESIGN TOKENS
 */
export const EASING = {
...
export const splitTextBilingual = (text, mode = 'words', lang = 'en') => {
    if (!text)
        return { original: '', items: [], type: mode };
    if (mode === 'lines') {
    ...
    if (lang === 'bn') {
        ...
        if (typeof IntlObj !== 'undefined' && 'Segmenter' in IntlObj) {
            try {
                ...
                items = Array.from(segmenter.segment(text)).map((seg) => seg.segment);
            }
            catch (e) {
 ...
 */
export const useMagnetic = (strength = 0.35, easeStrength = 0.15) => {
